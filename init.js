...
var AllCategoryNUM = {
  jp: [
    servents["saber"].length,
    servents["archer"].length,
    servents["lancer"].length - 1, //-1妖蘭
    servents["rider"].length,
    servents["caster"].length,
    servents["assassin"].length,
    servents["berserker"].length,
    servents["ruler"].length,
    servents["avenger"].length,
    servents["alterego"].length,
    servents["foreigner"].length,
    servents["mooncancer"].length,
    servents["pretender"].length,
  ],
  // 台服
  tw: [14, 11, 10, 13, 13, 9, 12, 7, 5, 6, 6, 3],
  // 自選
...
function getCheckedBtn(country) {
  if (country == "jp") return jpButton;
  if (country == "tw") return twButton;
  if (country == "z") return zButton;
  if (country == "sixth") return sixButton;
  if (country == "newyear_23_up") return newyearBtn_1;
  if (country == "newyear_23_down") return newyearBtn_2;
  return twButton;
}
...
function drawImage(x, y, image) {
  if (image.complete) {
    try {
      context.drawImage(image, x * (CELL_SIZE + col_padding) + marginLeft, y * (CELL_SIZE + row_padding) + marginTop, CELL_SIZE, CELL_SIZE);
      draw_done = 1;
    } catch (e) {
...
function fillCaculate() {
  ...
  for (var category = 0; category < CategoryLen; category++) {
    ...
    if (category <= default_cat2) {
      if (attribute > 0) {
        ...
        context.fillText("新:" + percent.toFixed(2) + "%", marginLeft - caculateField, marginTop + category * (CELL_SIZE + row_padding));

        percent = (haveFull / units[category].length) * 100;
        context.fillText("盤:" + percent.toFixed(2) + "%", marginLeft - caculateField, marginTop + category * (CELL_SIZE + row_padding) + 20);

        percent = (like / units[category].length) * 100;
        context.fillText("婆:" + percent.toFixed(2) + "%", marginLeft - caculateField, marginTop + category * (CELL_SIZE + row_padding) + 40);
      }
      // 新增雷
  ...
  if (lucky_bag == false) {
    ...
    context.fillText("新:" + percent.toFixed(2) + "%", marginLeft - caculateField, marginTop + 7 * (CELL_SIZE + row_padding));

    percent = (haveFull / ex) * 100;
    context.fillText("盤:" + percent.toFixed(2) + "%", marginLeft - caculateField, marginTop + 7 * (CELL_SIZE + row_padding) + 20);

    percent = (like / ex) * 100;
    context.fillText("婆:" + percent.toFixed(2) + "%", marginLeft - caculateField, marginTop + 7 * (CELL_SIZE + row_padding) + 40);
  }
  // 新增 雷
  // percent = (ban / ex * 100);
...
function fillNPText(x, y, msg, color) {
  ...
  if (number > 9) {
    context.fillText(msg, (x + 1) * (CELL_SIZE + col_padding) + 4 + marginLeft, (y + 1) * (CELL_SIZE + row_padding) + marginTop - row_padding + 5, CELL_SIZE);
  } else {
    context.fillText(msg, (x + 1) * (CELL_SIZE + col_padding) + 8 + marginLeft, (y + 1) * (CELL_SIZE + row_padding) + marginTop - row_padding + 5, CELL_SIZE);
  }
}
...
function fillTotalText() {
  ...
  context.fillStyle = font_color;
  context.fillText("英靈持有數:" + totalHave + "/" + total, canvas.width - width_fix, canvas.height - 90);
  context.fillText("英靈持有率:" + percent.toFixed(2) + "%", canvas.width - width_fix, canvas.height - 70);
  context.fillText("總寶數:" + totalNP, canvas.width - width_fix, canvas.height - 50);
}
...
function isOutPrint(point, attribute, category) {
  return (
    point.x - (attribute * (CELL_SIZE + col_padding) + marginLeft) < CELL_SIZE &&
